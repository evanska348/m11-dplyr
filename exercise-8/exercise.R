...
Pulitzer.Price.Change <- mutate(pulitzer, difference.1990.2003 = Pulitzer.Prize.Winners.and.Finalists..2004.2014 - 
...
# Be sure to use the pipe operator! 
  most.2014 <- filter(pulitzer, max(Pulitzer.Prize.Winners.and.Finalists..2004.2014) == Pulitzer.Prize.Winners.and.Finalists..2004.2014) %>% select(Newspaper)
...
# This publication should have Pulitzer prizes won a minimum of 5 Pulitzers, as well as the biggest decrease in circulation
  most.award.winning.decrease <- filter(pulitzer, Pulitzer.Prize.Winners.and.Finalists..2004.2014 >= 5) %>% filter(min(Change.in.Daily.Circulation..2004.2013 ) == Change.in.Daily.Circulation..2004.2013) %>% select(Newspaper)
...
#find publivation that gaiened the most between 1990 and 2014
  most.1990.2014 <- filter(pulitzer, max(Pulitzer.Prize.Winners.and.Finalists..1990.2014) == Pulitzer.Prize.Winners.and.Finalists..1990.2014) %>% select(Newspaper)
